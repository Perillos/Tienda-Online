...
<form name="updateproduct" method="post" action="/controller/update.product.controller.php?id=<?php echo $id_product; ?>" enctype="multipart/form-data">
    ...
    <div>
        ...
        echo $product_array[1]; ?>" required>
    </div>

    <div>
        <label for="priceproduct">Precio del producto (€):</label>
        <input name="priceproduct" id="priceproduct" type="number" value="<?php
        echo $product_array[4]; ?>" step="0.01" min="1" max="9999" required>
    </div>

    <div>
        <label for="typeproduct">Tipo de producto:</label>
        <select name="typeproduct" id="typeproduct" required>
            <option value="Sombrero" <?php $selected = ($product_array[3] == "Sombrero") ? "selected" : ""; echo $selected; ?>>Sombrero</option>
            <option value="Camisa" <?php $selected = ($product_array[3] == "Camisa") ? "selected" : ""; echo $selected; ?>>Camisa</option>
            <option value="Pantalon" <?php $selected = ($product_array[3] == "Pantalon") ? "selected" : ""; echo $selected; ?>>Pantalon</option>
            <option value="Calcetines" <?php $selected = ($product_array[3] == "Calcetines") ? "selected" : ""; echo $selected; ?>>Calcetines</option>
        </select>
    </div>

    <div>
        <label for="descriptionproduct">Descripción del producto:</label>
        <textarea  name="descriptionproduct" id="descriptionproduct" required><?php echo $product_array[2]; ?>
        </textarea>
    </div>
